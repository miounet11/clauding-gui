...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const loadAgents = async () => {
    try {
      ...
      setError(t('agents:messages.loadFailed'));
  ...
  const confirmDeleteAgent = async () => {
    ...
    try {
      ...
      setToast({ message: t('agents:messages.deleteSuccess'), type: "success" });
      await loadAgents();
      await loadRuns(); // Reload runs as they might be affected
    } catch (err) {
      console.error("Failed to delete agent:", err);
      setToast({ message: t('agents:messages.deleteFailed'), type: "error" });
    } finally {
      setIsDeleting(false);
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('agents:messages.createSuccess'), type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('agents:messages.updateSuccess'), type: "success" });
  };
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      setToast({ message: t('agents:messages.exportSuccess').replace('{{name}}', agent.name), type: "success" });
    } catch (err) {
      console.error("Failed to export agent:", err);
      setToast({ message: t('agents:messages.exportFailed'), type: "error" });
    }
  };
  ...
  const handleImportAgent = async () => {
    try {
      ...
      setToast({ message: t('agents:messages.importSuccess'), type: "success" });
      await loadAgents();
    } catch (err) {
    ...
    <div className={cn("flex flex-col h-full bg-background", className)}>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">{t('agents:header.title')}</h1>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={handleImportAgent}>
                    ...
                    {t('agents:importMenu.fromFile')}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setShowGitHubBrowser(true)}>
                    <Globe className="h-4 w-4 mr-2" />
                    {t('agents:importMenu.fromGitHub')}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              <Button
              ...
              <div>
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="text-lg font-medium mb-2">{t('agents:emptyState.title')}</h3>
                            ...
                            <Card className="h-full hover:shadow-lg transition-shadow">
                              <CardContent className="p-6 flex flex-col items-center text-center">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {t('agents:agentCard.created')} {new Date(agent.created_at).toLocaleDateString()}
                                </p>
                              </CardContent>
                    ...
                    {totalPages > 1 && (
                      <div className="mt-6 flex justify-center gap-2">
                        ...
                        <span className="flex items-center px-3 text-sm">
                          {t('agents:pagination.pageInfo').replace('{{current}}', currentPage.toString()).replace('{{total}}', totalPages.toString())}
                        </span>
                        <Button
                          size="sm"
                          variant="outline"
              ...
              {!loading && agents.length > 0 && (
                <div className="overflow-hidden">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-lg font-semibold">{t('agents:recentExecutions.title')}</h2>
                  </div>
                  {runsLoading ? (
      ...
      <GitHubAgentBrowser
        ...
        onImportSuccess={async () => {
          ...
          await loadAgents();
          setToast({ message: t('agents:messages.importFromGitHubSuccess'), type: "success" });
        }}
      />
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t('agents:deleteDialog.description').replace('{{name}}', agentToDelete?.name || '')}
            </DialogDescription>
          </DialogHeader>
            ...
            >
              {isDeleting ? (
                ...
                <>
                  ...
                  {t('agents:deleteDialog.confirmButton')}
                </>
              )}
            </Button>
          </DialogFooter>
