...
export const UsageDashboard: React.FC<UsageDashboardProps> = ({ onBack }) => {
  ...
  const getModelDisplayName = (model: string): string => {
    const modelKey = model.replace(/-/g, '').replace(/\./g, '').replace('claude', 'claude');
    const translationKey = `usage:models.${modelKey}`;
    const translated = t(translationKey);
    // If translation is not found, use the fallback map
      ...
      >
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-lg font-semibold">{t('usage:header.title')}</h1>
              <p className="text-xs text-muted-foreground">
                {t('usage:header.subtitle')}
              </p>
            </div>
          </div>
            ...
            <div className="flex space-x-1">
              {(["all", "30d", "7d"] as const).map((range) => (
                <Button
                  ...
                  className="text-xs"
                >
                  {range === "all" ? t('usage:dateRange.allTime') : range === "7d" ? t('usage:dateRange.last7Days') : t('usage:dateRange.last30Days')}
                </Button>
              ))}
            </div>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('usage:cards.totalCost')}</p>
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('usage:cards.totalSessions')}</p>
                    <p className="text-2xl font-bold mt-1">
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('usage:cards.totalTokens')}</p>
                    <p className="text-2xl font-bold mt-1">
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('usage:cards.avgCostPerSession')}</p>
                    <p className="text-2xl font-bold mt-1">
                      {formatCurrency(
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-5">
                ...
                <TabsTrigger value="projects">{t('usage:tabs.projects')}</TabsTrigger>
                <TabsTrigger value="sessions">{t('usage:tabs.sessions')}</TabsTrigger>
                <TabsTrigger value="timeline">{t('usage:tabs.timeline')}</TabsTrigger>
              </TabsList>

              {/* Overview Tab */}
              <TabsContent value="overview" className="space-y-4">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t('usage:overview.tokenBreakdown')}</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                      <p className="text-xs text-muted-foreground">{t('usage:overview.inputTokens')}</p>
                      <p className="text-lg font-semibold">{formatTokens(stats.total_input_tokens)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{t('usage:overview.outputTokens')}</p>
                      <p className="text-lg font-semibold">{formatTokens(stats.total_output_tokens)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{t('usage:overview.cacheWrite')}</p>
                      <p className="text-lg font-semibold">{formatTokens(stats.total_cache_creation_tokens)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{t('usage:overview.cacheRead')}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card className="p-6">
                    <h3 className="text-sm font-semibold mb-4">{t('usage:overview.mostUsedModels')}</h3>
                        ...
                        <div key={model.model} className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {model.session_count} {t('usage:overview.sessions')}
                            </span>
                          </div>
                  ...
                  <Card className="p-6">
                    <h3 className="text-sm font-semibold mb-4">{t('usage:overview.topProjects')}</h3>
                    <div className="space-y-3">
                      {stats.by_project.slice(0, 3).map((project) => (
                        <div key={project.project_path} className="flex items-center justify-between">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {project.session_count} {t('usage:overview.sessions')}
                            </span>
                          </div>
                          <span className="text-sm font-medium">
              ...
              <TabsContent value="models">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t('usage:models.title')}</h3>
                    ...
                    {stats.by_model.map((model) => (
                      <div key={model.model} className="space-y-2">
                        ...
                        <div className="grid grid-cols-4 gap-2 text-xs">
                          <div>
                            <span className="text-muted-foreground">{t('usage:models.input')} </span>
                            <span className="font-medium">{formatTokens(model.input_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('usage:models.output')} </span>
                            <span className="font-medium">{formatTokens(model.output_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('usage:models.cacheW')} </span>
                            <span className="font-medium">{formatTokens(model.cache_creation_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('usage:models.cacheR')} </span>
              ...
              <TabsContent value="projects">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t('usage:projects.title')}</h3>
                  <div className="space-y-3">
                    {stats.by_project.map((project) => (
                      <div key={project.project_path} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {formatCurrency(project.total_cost / project.session_count)}{t('usage:projects.perSession')}
                          </p>
                        </div>
                      </div>
                    ))}
              ...
              <TabsContent value="sessions">
                  <Card className="p-6">
                      <h3 className="text-sm font-semibold mb-4">{t('usage:sessions.title')}</h3>
                      <div className="space-y-3">
                    ...
                    return (
                      <div className="relative pl-8 pr-4">
                                ...
                                <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                                  <div className="bg-background border border-border rounded-lg shadow-lg p-3 whitespace-nowrap">
                                    ...
                                    <p className="text-sm text-muted-foreground mt-1">
                                      {t('usage:timeline.cost')} {formatCurrency(day.total_cost)}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                      {formatTokens(day.total_tokens)} {t('usage:tokens')}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                      {day.models_used.length} {t('usage:timeline.models')}{day.models_used.length !== 1 ? 's' : ''}
                                    </p>
                                  </div>
                        ...
                        <div className="mt-8 text-center text-xs text-muted-foreground">
                          {t('usage:timeline.dailyUsage')}
                        </div>
                      </div>
                    )
                  })() : (
                    <div className="text-center py-8 text-sm text-muted-foreground">
                      {t('usage:timeline.noData')}
                    </div>
                  )}
                </Card>
              </TabsContent>
